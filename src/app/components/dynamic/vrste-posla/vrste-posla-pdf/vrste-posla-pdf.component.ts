...
export class VrstePoslaPdfComponent {
  ...
  public generatePDF(columns: string[], data: any, numberOfColumns: number, title: string): void {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (this.dialogData.headers.includes('SIF_VP') || this.allColumns) {
        dataAsArray[i].push(data[i].SIF_VP);
      }

      if (this.dialogData.headers.includes('NAZ_VP') || this.allColumns) {
        dataAsArray[i].push(data[i].NAZ_VP);
      }

      if (this.dialogData.headers.includes('SI') || this.allColumns) {
        dataAsArray[i].push(data[i].SI);
      }
    ...
    this.globalVar.headerTitle = 'Vrste Posla';
  ...
  public getData(selectedSearchOption: string): void {
    ...
    this.http.postWithParams(
      ...
      if (this.dialogData.headers.includes('SIF_VP') || this.allColumns) {
        columnsList.push(this.t.translate('SIF_VP'));
      }

      if (this.dialogData.headers.includes('NAZ_VP') || this.allColumns) {
        columnsList.push(this.t.translate('NAZ_VP'));
      }

      if (this.dialogData.headers.includes('SI') || this.allColumns) {
        columnsList.push(this.t.translate('SI'));
      }

      this.generatePDF(columnsList, this.zaposleni, columnsList.length, this.t.translate('VrstePosla'));
    });
